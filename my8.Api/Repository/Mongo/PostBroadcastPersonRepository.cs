...
namespace my8.Api.Repository.Mongo
{
    public class PostBroadcastPersonRepository:MongoRepositoryBase,IPostBroadcastPersonRepository
    {
        ...
        public async Task<List<PostBroadcastPerson>> GetByPerson(string personId,int skip,int limit)
        {
            ...
            List<PostBroadcastPerson> lstPost = await collection.Find($@"{{ReceiversId:'{personId}'}}").Sort("{PostTime:1}").Skip(skip).Limit(limit).ToListAsync();
            return lstPost;
        }
        ...
        public async Task<bool> HidePost(PostBroadcastPerson post)
        {
            try
            {
                ...
                if (moveResult == true)
                {
                    ...
                    filter = filterBuilder.Eq(p => p.PostId, post.PostId) & filterBuilder.Eq(p => p.ReceiversId, post.ReceiversId) & filterBuilder.Eq(p => p.PostType, post.PostType);
